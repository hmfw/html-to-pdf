...
- **代码重构**：重构 `renderTextNode` 函数，提升代码质量和可维护性
  - 将 163 行的单一函数拆分为 6 个职责单一的小函数
  - 新增 `collectTextStyles` / `isInPreformatted` / `calculateAlignedX` / `renderSingleLine` / `renderPreformattedText` / `renderNormalText`
  - 函数名清晰表达意图，更易理解和测试
  - `renderTextNode` 简化为 30 行的高层协调器
- **类型优化**：统一文本渲染相关的类型定义
  - 提取基础类型 `BaseTextOptions`，包含 7 个共同字段
  - `DrawTextOptions` 和 `RenderWithFallbackOptions` 通过类型组合消除重复
  - 移除未使用的 `maxWidth` 和 `lineHeight` 字段
  - 类型层次更清晰，语义体现抽象层级
...
- **后备字体改为按需加载**：使用自定义字体（`fontPaths`）时，不再无条件预下载思源黑体。现在先扫描内容并子集化主字体，只有**真正检测到缺字**时才下载后备字体。自定义字体完整覆盖所用字符时，零额外字体请求（此前会白白下载 16-17MB）。
  - 行为向后兼容，API 不变；`fontFallback: false` 仍然完全关闭后备字体。
  - 注意：关闭子集化（`fontSubset: false`）时无法预知缺字，若启用后备仍会预加载完整后备字体。
