...
如果导出的 PDF 中某些字符显示为方块，通常是因为这些字符在字体中不存在。

**使用自定义字体时**（推荐）：
- 库会自动加载思源黑体作为后备字体
- 自定义字体缺少的字符会自动用后备字体渲染
- 控制台会提示：`[html-to-pdf] 主字体缺少 N 个字符，使用后备字体（思源黑体）补充`
- 正常情况下不会出现方块

**使用思源黑体时（默认）**：
如果仍出现方块，检查浏览器控制台是否有类似警告：
```
[html-to-pdf] 以下字符在字体 Source Han Sans SC Regular 中不存在，将使用后备字体：
'某' (U+67D0), '些' (U+4E9B)
```
...
3. **关闭子集化**：临时使用 `fontSubset: false` 嵌入完整字体（会增大文件约 16MB）
