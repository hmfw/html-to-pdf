...
`@hmfw/html-to-pdf` 的核心是**框架无关**的：真正生成 PDF 的 `exportToPdf` / `downloadPdf` 只接收一个原生 DOM 元素，内部读取 DOM 的真实布局（`getBoundingClientRect` / `Range`）绘制矢量内容，不依赖任何 UI 框架。

包只有一个入口 `@hmfw/html-to-pdf`，同时导出：

| 导出 | 内容 | 是否需要 Vue |
| --- | --- | --- |
| 工具函数 | `exportToPdf` / `downloadPdf` | **不需要** |
| 常量 / 类型 | `PDF_CONTAINER_ATTR` / `PDF_PAGE_ATTR` / 类型 | **不需要** |
| Vue3 组件 | `<PdfDocument>` / `<PdfPage>` + 插件 | 需要 `vue@^3.3.0` |

> React / Vue2 / 原生 JS 只引入工具函数与常量即可，不引入组件就不会牵入 Vue 依赖（会被 tree-shake 掉）。
...
2. 调用 `exportToPdf(element, options)` 得到 `{ success, blob, error }`。
3. 用 `downloadPdf(blob, filename)` 触发下载，或自行处理 `blob`（上传、预览等）。

### 分页（框架无关）

Vue3 里 `<PdfDocument>` / `<PdfPage>` 的能力，本质上就是往 DOM 上加两个 data 属性，任何框架都能手动复现：

- 导出根元素加 `data-pdf`
- 每个分页块加 `data-pdf-page="1"`、`data-pdf-page="2"`……（值从 1 递增）
...
利用 `useRef` 拿到 DOM 元素，调用 core 工具函数即可：
...
export function Report() {
  ...
  const handleExport = async () => {
    ...
    if (result.success && result.blob) {
      downloadPdf(result.blob, 'report')
      setStatus('success')
    } else {
...
Vue2 无法使用本库的 `.vue` 组件（它们用 Vue3 `<script setup>` 编写），但可直接调用 core 工具函数，用 `ref` 拿元素：
...
export default {
  ...
  methods: {
    async handleExport() {
      ...
      try {
        const result = await exportToPdf(this.$refs.content, { filename: 'document' })
        if (result.success && result.blob) {
          downloadPdf(result.blob, 'document')
        } else {
          console.error(result.error)
        }
      } finally {
...
Angular、Svelte、Solid 等同理：拿到模板中的 DOM 元素引用（`@ViewChild` / `bind:this` / `ref` 等），传给 `exportToPdf` 即可。
...
- 支持的 HTML/CSS 范围与 Vue3 用法一致，详见根目录 README。
